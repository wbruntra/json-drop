import { getGitHubAuthUrl, exchangeGitHubCode, getGitHubUser, createSession, setSessionCookie, generateState } from "../auth";
...
export async function handleGitHubCallback(req: Request): Promise<Response> {
  ...
  if (error) {
    return new Response(null, {
      status: 302,
      headers: { Location: `${frontendUrl}/?error=${encodeURIComponent(error)}`, ...corsHeaders() },
    });
  }
