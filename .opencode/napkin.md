...
- Frontend/Backend mismatch RESOLVED (2026-06-16): Redesigned dashboard around projects. Added `ProjectSelector` (create/list/select/delete) in the header. `Dashboard` now holds `projectId` state and passes it to `DocManager` and `TokenManager`. `DocManager` scopes list/create/update/delete via `?project=<id>` and uses path-based delete (`DELETE /api/docs?path=...`). `TokenManager` filters by `project_id` and includes it when creating tokens. `CurlGuide` now shows project-aware examples. Legacy id-based doc addressing removed from the frontend.
